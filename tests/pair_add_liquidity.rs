    ...
    F: FnOnce(TestEnvironment, &mut PlazaPair, Bucket, Bucket) -> Result<(), RuntimeError> 
{
    ...
    let base_bucket = ResourceBuilder::new_fungible(OwnerRole::None) 
        .divisibility(18)
        .mint_initial_supply(50_000, &mut env)?;
...
fn initial_add_gives_correct_lp() -> Result<(), RuntimeError> {
    publish_and_setup(|
        ...
        quote_bucket: Bucket,
    | -> Result<(), RuntimeError> {
        let (add_base, _) = pair.add_liquidity(base_bucket.take(dec!(10_000), &mut env)?, None, &mut env)?;
        let (add_quote, _) = pair.add_liquidity(quote_bucket.take(dec!(10_000), &mut env)?, None, &mut env)?;
        assert!(add_base.amount(&mut env)? == dec!(10_000), "Unexpected LP amount");
        assert!(add_quote.amount(&mut env)? == dec!(10_000), "Unexpected LP amount");
        Ok(())
    })
}
...
fn second_add_goes_in_ratio() -> Result<(), RuntimeError> {
    publish_and_setup(|
        ...
        quote_bucket: Bucket,
    | -> Result<(), RuntimeError> {
        let _ = pair.add_liquidity(base_bucket.take(dec!(10_000), &mut env)?, None, &mut env)?;
        let _ = pair.add_liquidity(quote_bucket.take(dec!(10_000), &mut env)?, None, &mut env)?;
        ...
        assert!(add_base.amount(&mut env)? == dec!(1000), "Unexpected LP amount: {}", add_base.amount(&mut env)?);
        assert!(add_quote.amount(&mut env)? == dec!(1000), "Unexpected LP amount: {}", add_quote.amount(&mut env)?);
        Ok(())
    })
}
...
fn correct_add_during_base_shortage() -> Result<(), RuntimeError> {
    publish_and_setup(|
        ...
        quote_bucket: Bucket,
    | -> Result<(), RuntimeError> {
        let (initial, _) = pair.add_liquidity(base_bucket.take(dec!(10_000), &mut env)?, None, &mut env)?;
        let _swap = pair.swap(quote_bucket.take(dec!(10_000), &mut env)?, &mut env)?;
        let (add_base, _) = pair.add_liquidity(base_bucket.take(dec!(5000), &mut env)?, Some(quote_bucket.take(dec!(10_000), &mut env)?), &mut env)?;
        ...
        assert!(rounded == dec!(10_000), "Unexpected LP amount: {}", rounded);
...
fn correct_add_during_quote_shortage() -> Result<(), RuntimeError> {
    publish_and_setup(|
        ...
        quote_bucket: Bucket,
    | -> Result<(), RuntimeError> {
        let (initial, _) = pair.add_liquidity(quote_bucket.take(dec!(10_000), &mut env)?, None, &mut env)?;
        let _swap = pair.swap(base_bucket.take(dec!(10_000), &mut env)?, &mut env)?;
        let (add_quote, _) = pair.add_liquidity(quote_bucket.take(dec!(5000), &mut env)?, Some(base_bucket.take(dec!(10_000), &mut env)?), &mut env)?;
        ...
        assert!(new_amount.checked_round(
            12,
            RoundingMode::ToNearestMidpointAwayFromZero
        ).unwrap() == dec!(10_000), "Unexpected LP amount");
