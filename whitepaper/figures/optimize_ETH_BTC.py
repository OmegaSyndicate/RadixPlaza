...
if __name__ == "__main__":
    ...
    data = pd.read_csv('ETHBTC_1h.csv', skiprows=1).dropna().sort_values(by='unix')
    ...
    #result = minimize(sortino_ratio, [0.7, 0.6, 3], (df), bounds=[(0.05, 2), (0, 2), (1, 4)])
    #print(result.x)
    
    INITIAL_BASE = 0.5 / df.loc[df.index[-1], 'close']
    INITIAL_QUOTE = 0.5
    initial_state = ExchangeState(
